...
class GCNConv3(MessagePassing):
    """
    修改 aggregate 后的 图神经网络
    """
    def __init__(self, in_channels, out_channels):
        super(GCNConv3, self).__init__(aggr='add', flow='source_to_target')
        # Add 是 聚合操作
        # flow 是 流向，从源节点到目标节点传播信息
...
net = GCNConv3(data.num_features, 64)  # 初始化网络
h_nodes = net(data.x, data.edge_index)  # 运行 图神经网络
print(h_nodes.shape)  # 打印信息
# torch.Size([2708, 64])
